...
export default function ComplaintsManagementPage() {
  ...
  const closeItem = async (id) => {
    const ok = window.confirm('Close this escalation and delete it permanently?');
    if (!ok) return;
  ...
  return (
    <Layout title="Escalations Management">
      <div className="hero-strip mb-4 p-5">
        <p className="ui-hand-label mb-3 inline-block">case board</p>
        <h2 className="text-2xl font-bold">Escalations Management</h2>
        <p className="mt-1 text-sm text-slate-100">Track escalated issues, validate repeated cases, and improve response quality.</p>
      </div>
